  ...
  <TabsRoot v-model="currentTab" class="bg-neutral-900" @update:model-value="open = true">
    <div class="bg-neutral-900 border-b border-neutral-800 flex pr-2 text-sm">
      <div class="flex justify-between items-center w-full">
        <TabsList class="flex">
          <TabsTrigger v-for="(tab, index) in tabs" :key="index" :value="tab.label" tabindex="-1"
    ...
    <div ref="codeScrollWrapper" class="bg-neutral-900 pb-10 block max-h-[80vh]"
      :class="`${open ? 'overflow-scroll' : 'overflow-hidden'}`" :style="{ height: height }">
