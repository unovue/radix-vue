...
This documentation provides guidance on how to utilize multi-directional support in Radix UI with SSR support. Radix Primitives rely on [`FloatingUI`](https://floating-ui.com/) to position floating elements, which requires to be fed the current direction of the web app.
...
Some languages are written from LTR and others are written in RTL. In a multi-language web app, you need to configure directionality alongside the translations. This is a simplified guide on how to achieve that using `radix-ui` primitives.
