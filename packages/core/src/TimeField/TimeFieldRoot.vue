<script lang="ts">
import { type DateValue, getLocalTimeZone, isEqualDay, toCalendarDateTime, today } from '@internationalized/date'
...
import { createContent, getTimeFieldSegmentElements, initializeSegmentValues, isSegmentNavigationKey, syncSegmentValues } from '../DateField/utils'
...
const convertedModelValue = computed(() => convertValue(modelValue.value))
...
const convertedPlaceholder = computed(() => convertValue(placeholder.value))
...
const initialSegments = initializeSegmentValues(inferredGranularity.value, true)

const segmentValues = ref<SegmentValueObj>(modelValue.value ? { ...syncSegmentValues({ value: convertedModelValue.value, formatter }, true) } : { ...initialSegments })
...
watch([convertedModelValue, locale], ([_modelValue]) => {
  if (_modelValue !== undefined) {
    segmentValues.value = { ...syncSegmentValues({ value: _modelValue, formatter }, true) }
  }
