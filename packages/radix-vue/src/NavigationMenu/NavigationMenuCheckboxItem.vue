...
interface NavigationMenuCheckboxItemProps {
  asChild?: boolean;
  checked?: boolean;
...
import { NAVIGATION_MENU_ITEM_SYMBOL } from "./utils";
import {
  NAVIGATION_MENU_INJECTION_KEY,
  type NavigationMenuProvideValue,
} from "./NavigationMenuRoot.vue";

const injectedValue = inject<NavigationMenuProvideValue>(
  NAVIGATION_MENU_INJECTION_KEY
);

const props = defineProps<NavigationMenuCheckboxItemProps>();
...
provide(NAVIGATION_MENU_ITEM_SYMBOL, {
  modelValue,
});
