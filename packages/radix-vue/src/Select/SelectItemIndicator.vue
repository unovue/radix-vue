...
const selectItemInjectedValue = inject<SelectItemProvideValue>(SELECT_ITEM_SYMBOL);

const visibleState = computed(() => {
  if (rootInjectedValue?.multiple) {
    return rootInjectedValue?.modelValue.value?.includes(selectItemInjectedValue!.value)
  } else {
    return rootInjectedValue?.modelValue.value === selectItemInjectedValue?.value
  }
})
</script>
